import streamlit as st
from streamlit_option_menu import option_menu
from sqlite_db import SQLite

def on_click_update(username, role):
    if username == st.session_state.text_username:
        st.session_state.text_role = role

def app(SQLiteDB:SQLite):
    if st.session_state.login_status and st.session_state.role != 'admin':
        st.error('User is not Authorized..')
    elif not st.session_state.login_status:
        message_string = '''
                        Please login with admin account to register any user.
                        Please click on Login if you are not admin to access the application
                        '''
        st.info(message_string)
    else:
        _, col2, _ = st.columns([0.25, 0.5, 0.25])
        with col2.container(border=True):
            options_selected = option_menu(
                            menu_title = None,
                            options=['Update', 'Delete'],
                            icons = ['person-lines-fill', 'person-x'],
                            orientation='horizontal',
                            default_index=0,
                            styles={
                                        "container": {"padding": "0!important", "background-color": "#34495E"},
                                        "icon": {"color": "white", "font-size": "20px"},
                                        "nav-link": {
                                            "font-size": "20px",
                                            "font-family": "system-ui",
                                            "text-align": "center",
                                            "margin": "5px",
                                            "font-weight": "bold"
                                        },
                                        "nav-link-selected": {"background-color": "#28B463"}
                                    }        
                            )
            if options_selected == 'Update':
                with col2.form(key='Update'):
                    username = st.text_input(label='Username')
                    role = st.text_input(label='Role')
                    update_button = st.form_submit_button('Update', on_click=on_click_update(username, role))
                    if update_button:
                        result = SQLiteDB.update_user_role(username, role)
                        if result is not None:
                            st.info(f'User - {result[0]} Role Updated Successfully to {result[3]}')
                        else:
                            st.error('No User Present')
            elif options_selected == 'Delete':
                with col2.form(key='Update'):
                    username = st.text_input(label='Username')
                    delete_button = st.form_submit_button('Delete')
                    if delete_button:
                        result = SQLiteDB.delete_user(username)
                        if result is None:
                            st.error(f'User - {username} deleted Successfully..')
